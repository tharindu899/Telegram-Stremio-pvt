...
async def subtitle_handler(
    ...
    if not msg_id:
        raise HTTPException(status_code=400, detail="Missing id")
    chat_id = int(f"-100{decoded['chat_id']}")
    return await subtitle_streamer(
        request=request,
        chat_id=chat_id,
        msg_id=int(msg_id),
        token=token,
...
async def subtitle_streamer(
    ...
    parallelism = min(Telegram.PARALLEL, 2)
    prefetch_count = Telegram.PRE_FETCH
    extra_clients_for_stream = []
    if parallelism > 1 and len(multi_clients) > 1:
        other_indices = sorted(
            (i for i in multi_clients if i != index),
    ...
    file_name = file_id.file_name or f"{secrets.token_hex(4)}.srt"
    mime_type = get_subtitle_mime_type(file_name)
    from fastapi.responses import Response as PlainResponse
    if request.method == "HEAD":
        headers = {
            "Content-Type": mime_type,
...
def get_subtitle_mime_type(filename: str) -> str:
    ...
    elif filename.endswith('.srt'): return "application/x-subrip"
    elif filename.endswith(('.ass','.ssa')): return "text/x-ssa"
    elif filename.endswith('.sub'): return "text/x-microdvd"
    return "text/plain"

@router.get("/sub/{token}/{id}/{name}")
@router.head("/sub/{token}/{id}/{name}")
async def subtitle_handler(request: Request, token: str, id: str, name: str, token_data: dict = Depends(verify_token)):
    decoded = await decode_string(id)
    msg_id = decoded.get("msg_id")
    if not msg_id: raise HTTPException(status_code=400, detail="Missing id")
    chat_id = int(f"-100{decoded['chat_id']}")
    return await subtitle_streamer(request=request, chat_id=chat_id, msg_id=int(msg_id), token=token, token_data=token_data, stream_id_hash=id)

async def subtitle_streamer(request: Request, chat_id: int, msg_id: int, token: str, token_data: dict = None, stream_id_hash: str = None):
    index = select_best_client(0)
    tg_client = multi_clients[index]
    if tg_client not in _streamer_by_client: _streamer_by_client[tg_client] = ByteStreamer(tg_client, index)
    streamer = _streamer_by_client[tg_client]
    file_id = await streamer.get_file_properties(chat_id=chat_id, message_id=msg_id)
    file_size = file_id.file_size
    range_header = request.headers.get("Range", "")
    start, end = parse_range_header(range_header, file_size)
    ...
    if stream_id_hash:
        _now = time.time()
        _cached = _title_cache.get(stream_id_hash)
        if _cached and _now < _cached[1]: db_title = _cached[0]
        else: db_title = await db.get_title_by_stream_id(stream_id_hash); _title_cache[stream_id_hash] = (db_title, _now + _TITLE_CACHE_TTL)
    final_title = db_title if db_title else decoded_name
    meta = {"request_path": str(request.url.path), "client_host": request.client.host if request.client else None, "title": final_title, "user_name": token_data.get("name","Unknown") if token_data else "Unknown", "type": "subtitle"}
    parallelism = min(Telegram.PARALLEL, 2)
    prefetch_count = Telegram.PRE_FETCH
    extra_clients_for_stream = []
    if parallelism > 1 and len(multi_clients) > 1:
        other_indices = sorted((i for i in multi_clients if i != index), key=lambda i: work_loads.get(i, 0))
        async def _get_extra(ec_idx: int):
            ec_client = multi_clients[ec_idx]
            if ec_client not in _streamer_by_client: _streamer_by_client[ec_client] = ByteStreamer(ec_client, ec_idx)
            try: return (ec_idx, _streamer_by_client[ec_client], await _streamer_by_client[ec_client].get_file_properties(chat_id=chat_id, message_id=msg_id))
            except: return None
        results = await asyncio.gather(*[_get_extra(i) for i in other_indices[:parallelism-1]])
        extra_clients_for_stream = [r for r in results if r]
    body_gen = await streamer.prefetch_stream(file_id=file_id, client_index=index, offset=offset, first_part_cut=first_part_cut, last_part_cut=last_part_cut, part_count=part_count, chunk_size=chunk_size, prefetch=prefetch_count, stream_id=stream_id, meta=meta, parallelism=parallelism, request=request, chat_id=chat_id, message_id=msg_id, extra_clients=extra_clients_for_stream)
    file_name = file_id.file_name or f"{secrets.token_hex(4)}.srt"
    mime_type = get_subtitle_mime_type(file_name)
    from fastapi.responses import Response as PlainResponse
    if request.method == "HEAD":
        headers = {"Content-Type": mime_type, "Content-Length": str(req_length), "Content-Disposition": f'inline; filename=\"{file_name}\"', "Accept-Ranges": "bytes", "Cache-Control": "public, max-age=86400", "Access-Control-Allow-Origin": "*", "Access-Control-Expose-Headers": "Content-Length,Content-Range,Accept-Ranges"}
    ...
    headers = {"Content-Type": mime_type, "Content-Disposition": f'inline; filename=\"{file_name}\"', "Accept-Ranges": "bytes", "Content-Length": str(req_length), "Cache-Control": "public, max-age=86400", "Access-Control-Allow-Origin": "*", "Access-Control-Expose-Headers": "Content-Length,Content-Range,Accept-Ranges"}
    if range_header: headers["Content-Range"] = f"bytes {start}-{end}/{file_size}"
    return StreamingResponse(body_gen, headers=headers, status_code=206 if range_header else 200, media_type=mime_type)
