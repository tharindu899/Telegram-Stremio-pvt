"""
Source Manager Plugin — add, remove, and list AUTH_CHANNELs / topic groups via the bot.

Commands (owner-only, private chat):
    /channels               — List all active AUTH_CHANNELs / topic groups with names
    /addchannel <id>        — Add a channel or forum topic group parent ID
...
async def list_channels(client: Client, message: Message):
    if not Telegram.AUTH_CHANNEL:
        await message.reply_text(
            "📭 No AUTH_CHANNELs / topic groups configured.\n\n"
            "Use /addchannel <code>&lt;chat_id&gt;</code> to add one.",
            quote=True,
...
async def add_channel(client: Client, message: Message):
    ...
    try:
        ...
        await message.reply_text(
            ...
            "Chat IDs are numeric, usually starting with <code>-100</code>. For topic groups use the parent group ID only.",
            quote=True,
            parse_mode=enums.ParseMode.HTML,
        )
        return
    ...
    if ch_id in Telegram.AUTH_CHANNEL:
        await message.reply_text(
            f"ℹ️ Source <code>{ch_id}</code> is already in AUTH_CHANNELs.",
            quote=True,
            parse_mode=enums.ParseMode.HTML,
        )
        return
    ...
    try:
        ...
        await status_msg.edit_text(
            f"❌ <b>Access denied</b> to chat <code>{ch_id}</code>.\n\n"
            "Make sure the bot is added as an <b>admin</b> in the channel/group first.",
            parse_mode=enums.ParseMode.HTML,
        )
        return
    except PeerIdInvalid:
        await status_msg.edit_text(
            f"❌ Chat <code>{ch_id}</code> not found.\n"
            "Double-check the ID.",
            parse_mode=enums.ParseMode.HTML,
        )
        return
    ...
    await status_msg.edit_text(
        ...
        f"The bot will now index new videos/subtitles from this channel or topic group. For a forum group, all topics inside the parent group are supported.\n"
...
async def remove_channel(client: Client, message: Message):
    ...
    if len(args) < 2:
        if Telegram.AUTH_CHANNEL:
            ...
            await message.reply_text(
                "Usage: /removechannel <code>&lt;chat_id&gt;</code>\n\n"
    ...
    if ch_id not in Telegram.AUTH_CHANNEL:
        await message.reply_text(
            f"❌ Source <code>{ch_id}</code> is not in AUTH_CHANNELs.",
            quote=True,
            parse_mode=enums.ParseMode.HTML,
        )
        return
    ...
    await message.reply_text(
        f"✅ <b>Source removed!</b>\n\n"
        f"📡 <b>{ch_name}</b> (<code>{ch_id}</code>)\n\n"
        f"⚠️ Existing DB entries from this source are <b>not</b> deleted.\n"
