...
async def handle_payment_screenshot(client: Client, message: Message):
    ...
    try:
        LOGGER.debug(f"Payment screenshot handler triggered by {sender_id}")
        # Check if user has a pending payment request
        user = await db.get_user(sender_id)
        LOGGER.debug("Payment screenshot user record loaded")
        if not user or "pending_payment" not in user:
            # No active payment flow - tell the user what to do
            LOGGER.debug(f"No pending payment found for {sender_id}")
        ...
        for approver_id in approver_ids:
            try:
                ...
                LOGGER.warning(f"Failed to forward screenshot to approver {approver_id}: {e}")
