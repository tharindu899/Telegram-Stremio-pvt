...
async def run_auto_catalog_sync(db, *, force: bool = False, full_rebuild: bool = False, delay_seconds: int = 0) -> dict:
    ...
    async with _auto_sync_lock:
        ...
        if not settings_configured or not enabled_names:
            ...
            LOGGER.debug("Auto catalog sync skipped: no catalog selection saved yet.")
            return summary
        ...
        try:
            ...
            if full_rebuild:
                LOGGER.info(
                    "Auto catalog full rebuild complete: scanned=%s indexed=%s catalogs=%s",
                    scanned, tagged, len(catalog_items),
                )
            else:
                LOGGER.debug(
                    "Auto catalog quick sync complete: scanned=%s indexed=%s catalogs=%s",
                    scanned, tagged, len(catalog_items),
                )
            return summary
...
async def start_auto_catalog_interval_loop(db) -> None:
    ...
    selection_missing_reported = False
    while True:
        try:
            await asyncio.sleep(interval_seconds)

            if not await has_auto_catalog_settings(db):
                if not selection_missing_reported:
                    LOGGER.debug("Auto catalog interval waiting for WebUI catalog selection.")
                    selection_missing_reported = True
                continue

            selection_missing_reported = False
            if _auto_sync_lock.locked() or (_auto_sync_task and not _auto_sync_task.done()):
                LOGGER.debug("Auto catalog interval skipped because a sync is already running.")
                continue
            ...
            LOGGER.debug("Auto catalog interval queued: %s", result.get("mode", "quick_sync"))
