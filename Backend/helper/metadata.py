...
from Backend.helper.split_archive import split_zip_info, split_video_info, strip_split_zip_suffix, strip_split_video_suffix
...
def _infer_resolution_from_name(filename: str, parsed: dict | None = None) -> str | None:
    """Best-effort resolution fallback for messy Telegram names.

    PTN misses common mobile/channel filenames such as:
      Movie 2025 HDRip x264 700MB.mkv
      Show.S04E01.Title.720.mkv
      Anime.Movie.1080.10bit.Bluray.mkv

    Returning a conservative label keeps the scanner from skipping valid
    video files while still preferring real 2160p/1080p/720p/480p tags.
    """
    text = f" {filename or ''} "
    # real tags: 2160p / 1080p / 720p / 480p / 360p, also 1080. / 720_
    m = re.search(r"(?i)(?<!\d)(2160|1440|1080|720|576|540|480|360)\s*p?(?!\d)", text)
    if m:
        return f"{m.group(1)}p"

    # Some Telegram names are truncated near the extension: .108.mkv / .72.mkv
    # Treat those as common 1080p/720p abbreviations.
    low = text.lower()
    if re.search(r"(?:^|[ ._\-\[\(])108(?:[ ._\-\]\)]|\.(?:mkv|mp4|avi|mov)\b)", low):
        return "1080p"
    if re.search(r"(?:^|[ ._\-\[\(])72(?:[ ._\-\]\)]|\.(?:mkv|mp4|avi|mov)\b)", low):
        return "720p"

    enc = str((parsed or {}).get("encoder") or "").strip().lower()
    if enc in {"108", "1080", "1080p"}:
        return "1080p"
    if enc in {"72", "720", "720p"}:
        return "720p"

    # Size-only release names normally mean lower/medium quality.
    size_m = re.search(r"(?i)(\d+(?:\.\d+)?)\s*(gb|mb)\b", text)
    if size_m:
    ...
    # Last resort: index valid video filenames instead of losing them.
    if re.search(r"(?i)\.(mkv|mp4|avi|mov|webm)$", filename or ""):
        return "SD"
    return None


def _clean_title_resolution_leak(title: str, resolution: str | None) -> str:
    """Remove trailing resolution fragments when PTN put them into title."""
    if not title:
        return title
    cleaned = title
    # A Whisker Away 1080 -> A Whisker Away
    cleaned = re.sub(r"(?i)\s+(2160|1440|1080|720|576|540|480|360|108|72)\s*$", "", cleaned).strip(" ._-—")
...
# ----------------- Main Metadata -----------------
async def metadata(filename: str, channel: int, msg_id, override_id: str = None, require_quality: bool = True) -> dict | None:
    # Normal video path should not index individual split archive parts as playable files.
    # Split ZIP/direct video parts are grouped by the receiver/scanner first, then this
    # function is called with the clean base name.
    if split_zip_info(filename):
        LOGGER.debug(f"Skipping individual split ZIP part in metadata(): {filename}")
        return None
    if split_video_info(filename):
        LOGGER.debug(f"Skipping individual split video part in metadata(): {filename}")
        return None
    ...
    if "excess" in parsed and any("combined" in item.lower() for item in parsed["excess"]):
        LOGGER.debug(f"Skipping {filename}: contains 'combined'")
        return None
    ...
    if multipart_pattern.search(filename):
        LOGGER.debug(f"Skipping {filename}: seems to be a split/multipart file")
        return None
    ...
    resolution = parsed.get("resolution") or _infer_resolution_from_name(filename, parsed)
    title = _clean_title_resolution_leak(title, resolution)
    # Video files prefer a real resolution, but Telegram channel filenames are
    # often messy/truncated. If PTN misses it we infer from filename/size and
    # only fall back to SD as a last resort so scanner indexes valid videos.
    # Subtitle files can opt out with require_quality=False.
    quality = resolution
    if isinstance(season, list) or isinstance(episode, list):
        LOGGER.debug(f"Invalid season/episode format for {filename}: {parsed}")
        return None
    if season and not episode:
        LOGGER.debug(f"Missing episode in {filename}: {parsed}")
        return None
    if not quality:
        if require_quality:
            LOGGER.debug(f"Skipping {filename}: No resolution (parsed={parsed})")
            return None
        quality = parsed.get("quality") or "SUB"
    if not title:
        LOGGER.debug(f"No title parsed from: {filename} (parsed={parsed})")
        return None
    ...
    try:
        if season and episode:
            LOGGER.debug(f"Fetching TV metadata: {title} S{season}E{episode}")
            return await fetch_tv_metadata(title, season, episode, encoded_string, year, quality, default_id)
        else:
            LOGGER.debug(f"Fetching Movie metadata: {title} ({year})")
...
async def fetch_tv_metadata(title, season, episode, encoded_string, year=None, quality=None, default_id=None) -> dict | None:
    ...
    # =======================================================
    if must_use_tmdb:
        LOGGER.debug(f"No valid IMDb TV data for '{title}' → using TMDb")
...
async def fetch_movie_metadata(title, encoded_string, year=None, quality=None, default_id=None) -> dict | None:
    ...
    # =======================================================
    if must_use_tmdb:
        LOGGER.debug(f"No valid IMDb data for '{title}' → using TMDb")
